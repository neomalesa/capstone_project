...
const Dashboard = () => {
  ...
  const fetchListings = async () => {
    try {
      ...
      // If user is a host, only show their listings
  ...
  return (
    <>
      <Navbar />
      <div className="page-content" style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 20px' }}>
        <div style={{ display: 'flex', gap: '16px', marginBottom: '40px' }}>
          <Link to="/admin/reservations" style={{ padding: '10px 20px', border: '1px solid #dddddd', borderRadius: '24px', textDecoration: 'none', color: '#222222', fontSize: '14px', fontWeight: '500' }}>View Reservations</Link>
          <Link to="/admin/view-listings" style={{ padding: '10px 20px', border: '1px solid #dddddd', borderRadius: '24px', textDecoration: 'none', color: '#222222', fontSize: '14px', fontWeight: '500' }}>View Listings</Link>
          <Link to="/admin/create" style={{ padding: '10px 20px', border: '1px solid #dddddd', borderRadius: '24px', textDecoration: 'none', color: '#222222', fontSize: '14px', fontWeight: '500' }}>Create Listing</Link>
        </div>
        <h1 style={{ textAlign: 'center', fontSize: '24px', fontWeight: 'bold', marginBottom: '40px', color: '#222222' }}>My Listings</h1>

        {loading ? (
          <div style={{ textAlign: 'center' }}>Loading your listings...</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {listings.length === 0 && (
              <div style={{ textAlign: 'center', padding: '60px 20px', backgroundColor: '#f7f7f7', borderRadius: '12px' }}>
                ...
                <Link to="/admin/create" style={{ backgroundColor: '#e31c5f', color: 'white', padding: '12px 24px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold' }}>
                  Create Listing
                </Link>
              </div>
            )}
